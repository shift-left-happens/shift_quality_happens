...
public class EmployeeLeaveOverviewController {
    ...
    public List<EmployeeLeaveOverviewDto> getAllEmployeeLeaveOverview(Authentication auth) {
        if (authHelper.isEmployee(auth)) {
            return this.employeeLeaveOverviewRepository.findByEmployeeId(authHelper.currentEmployeeId(auth)).stream()
                    .map(EmployeeLeaveOverviewDto::from)
                    .toList();
        }
        return this.employeeLeaveOverviewRepository.findAll().stream()
                .map(EmployeeLeaveOverviewDto::from)
                .toList();
    }
    ...
    @PreAuthorize("isAuthenticated()")
    public List<EmployeeLeaveOverviewDto> getEmployeeLeaveOverviewByEmployeeId(
            @PathVariable Integer employeeId, Authentication auth) {
        if (authHelper.isEmployee(auth) && !employeeId.equals(authHelper.currentEmployeeId(auth))) {
            throw authHelper.forbidden();
        }
        return this.employeeLeaveOverviewRepository.findByEmployeeId(employeeId).stream()
                .map(EmployeeLeaveOverviewDto::from)
                .toList();
    }
    ...
    public List<EmployeeLeaveOverviewDto> getEmployeeLeaveOverviewByStatus(@PathVariable String status) {
        return this.employeeLeaveOverviewRepository.findByRequestStatus(status).stream()
                .map(EmployeeLeaveOverviewDto::from)
                .toList();
    }
    ...
    public List<EmployeeLeaveOverviewDto> getEmployeeLeaveOverviewByLeaveType(@PathVariable String leaveTypeName) {
        return this.employeeLeaveOverviewRepository.findByLeaveTypeName(leaveTypeName).stream()
                .map(EmployeeLeaveOverviewDto::from)
                .toList();
    }
}
