...
public class EmployeeShiftOverviewController {
    ...
    public List<EmployeeShiftOverviewDto> getEmployeeShiftOverview(Authentication auth) {
        if (authHelper.isEmployee(auth)) {
            return this.employeeShiftOverviewRepository.findByEmployeeId(authHelper.currentEmployeeId(auth)).stream()
                    .map(EmployeeShiftOverviewDto::from)
                    .toList();
        }
        return this.employeeShiftOverviewRepository.findAll().stream()
                .map(EmployeeShiftOverviewDto::from)
                .toList();
    }
    ...
    @PreAuthorize("isAuthenticated()")
    public List<EmployeeShiftOverviewDto> getEmployeeShiftOverviewByEmployeeId(
        ...
        return this.employeeShiftOverviewRepository.findByEmployeeId(employeeId).stream()
                .map(EmployeeShiftOverviewDto::from)
                .toList();
    }
}
